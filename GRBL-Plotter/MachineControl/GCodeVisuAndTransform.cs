...
namespace GRBL_Plotter
{
    ...
    {   public enum translate { None, ScaleX, ScaleY, Offset1, Offset2, Offset3, Offset4, Offset5, Offset6, Offset7, Offset8, Offset9, MirrorX, MirrorY };
        ...
        public void createDarwingPathFromGCode(byte motionMode, byte spindleState, double ox, double oy, double oz, double nx, double ny, double nz, double? ii, double? jj)
        {
            ...
            if ((oldSpindleState == 5) && ((spindleState == 3) || (spindleState == 4)))
            { path = pathPenDown; }
            if (((oldSpindleState == 3) || (oldSpindleState == 3)) && (spindleState == 5))
            { path = pathPenUp; }
            oldSpindleState = spindleState;

            if ((nz < oz) && (nz < zLimit))                    // select path to draw depending on Z-Value
            { path = pathPenDown; }
            if ((nz > oz) && (nz >= zLimit))
            { path = pathPenUp;  }
